...
def get_pipeline() -> type[transformers.Pipeline]:
    class InstructionTextGenerationPipeline(transformers.Pipeline):
        def __init__(
            ...
            **kwargs: t.Any,
            ...
            """
            super().__init__(
                *args, do_sample=do_sample, max_new_tokens=max_new_tokens, top_p=top_p, top_k=top_k, **kwargs
            )
...
class DollyV2(openllm.LLM["transformers.Pipeline", "transformers.PreTrainedTokenizer"]):
    ...
    def load_model(self, tag: bentoml.Tag, *args: t.Any, **attrs: t.Any) -> transformers.Pipeline:
        ...
        return get_pipeline()(model=model, tokenizer=_ref.custom_objects["tokenizer"], **kwds)
